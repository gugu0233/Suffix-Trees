  ...
  (let iter ((list list) (L '()) (maxim 0))
    (cond
      ...
      ((> (length (car list)) maxim)
       (iter (cdr list) (car list) (length (car list))))
      (else
...
(define (nrCar L)
  (apply + (map length L)))  ;f care numara caracterele din lista

(define (trans L) ; f care transforma lista din una cu mai multe liste in una simpla
  (apply append L))

(define (mainfunc ST len L)
  (cond
    ((null? ST) #f)
    ((>= (nrCar L) len) ; caz ideal
     (take (trans L) len))
    ((not (null? (get-branch-subtree (first-branch ST))))
     (let* ((primaR (first-branch ST))
            (rest (get-branch-subtree primaR))
            (nod (get-branch-label primaR))
            (rez (mainfunc rest len (append L (list nod)))))
       (if rez
           rez
           (mainfunc (get-branch-subtree ST) len L))))
    (else
     (mainfunc (other-branches ST) len L))))

(define (repeated-substring-of-given-length text len)
  (mainfunc ((text->st1 text) cst-func) len '()))
