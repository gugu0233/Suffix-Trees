...
(define (get-ch-words words ch)
  (filter (lambda (words) (and (not(null? words)) (equal? ch (car words)))) words))
...
(define (ast-func suffixes)
  (if (null? suffixes) null
  (cons (list(caar suffixes)) (map (lambda (suffixes) (cdr suffixes)) suffixes))))
...
(define (cst-func suffixes)
  (if (null? suffixes) null
  (cons (longest-common-prefix-of-list suffixes) (map (lambda (word) (drop word (length (longest-common-prefix-of-list suffixes)))) suffixes))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (filter (lambda (list) (not (null? list)))
        (map (lambda (ch)
               (let ((rez (get-ch-words suffixes ch)))
                 (if (or (null? rez) (null? (labeling-func rez)))
                     '()
                     (cons (car (labeling-func rez)) (suffixes->st labeling-func (cdr (labeling-func rez)) alphabet))))) alphabet)))
